...
class ShoppingBagAdapter(val context: Activity, val products: ArrayList<Product>): RecyclerView.Adapter<ShoppingBagAdapter.ShoppingBagViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ShoppingBagViewHolder, position: Int) {
        ...
        val product = products[position] // EACH OF THE CATEGORIES

        holder.textViewName.text = product.name
        holder.textViewCounter.text = "${product.quantity}"

        if (product.quantity != null) {
            holder.textViewPrice.text = "${product.price * product.quantity!!} VND"
        }
...
//        holder.itemView.setOnClickListener { goToDetail(product) }
    ...
    class ShoppingBagViewHolder(view: View): RecyclerView.ViewHolder(view) {
        ...
        val textViewCounter: TextView = view.findViewById(R.id.textview_counter)
        val imageViewProduct: ImageView = view.findViewById(R.id.imageview_product)
        val imageViewAdd: ImageView = view.findViewById(R.id.imageview_add)
        val imageViewRemove: ImageView = view.findViewById(R.id.imageview_remove)
