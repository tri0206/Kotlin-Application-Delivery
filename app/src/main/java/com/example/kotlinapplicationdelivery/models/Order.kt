...
data class Order @JvmOverloads constructor(
    ...
    override fun toString(): String {
        return "Order(id=$id, idClient='$idClient', idDelivery=$idDelivery, idAddress='$idAddress', idRestaurant='$idRestaurant', status=$status, timestamp=$timestamp, products=$products, note = $note, payment = $payment, client=$client, delivery=$delivery, address=$address)"
    }
}